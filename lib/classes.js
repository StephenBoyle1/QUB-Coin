...
function getTransfersFor(authenticatedUser){
    ...
    console.log("Retrieving [%s] transfers for user [%s]", authenticatedUser.numOfTransfers, authenticatedUser.accountId);

    for(var index=1; index <= authenticatedUser.numOfTransfers; index++){   // Starting INDEX at 1 here!!!
        transfersList.push(
            convertRawTransferDataToJson(
                authenticatedUser,
                qubCoinContract().getTransferAtIndex(index, {from: authenticatedUser.accountId})
            )
        );
    }
