...
function logAttendanceFor(fromUser, secret, classId, callback) {
    qubCoinContract().logAttendance(classId, secret,
        {from: fromUser, gas:1000000}, function(err, data) {
            if (err) {
...
function getTransfersFor(authenticatedUser){
    ...
    for(var index=1; index <= authenticatedUser.numOfTransfers; index++){   // Starting INDEX at 1 here!!!
        transfersList.push(
            convertRawTransferDataToJson(qubCoinContract().getTransferAtIndex(index))
        )
    }
...
function convertRawTransferDataToJson(rawTransfer){
    console.log("converting rawTransfer data: [%s]", rawTransfer);
    return {
        isDebit: rawTransfer[0],
        sender: rawTransfer[1].toString(),
        receiver: rawTransfer[2].toString(),
        classId: rawTransfer[3].toString(),
...
function parseToFeedbackRateEnum(rawFeedback) {
    if(parseInt(rawFeedback) == FeedbackRateEnum.Average){
        return "Average";
    } else {
        return "Feedback";
    }
